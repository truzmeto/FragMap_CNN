...
for ibatch in range(nsample):
    ...
    rot_gfe = get_24(torch.tensor(target), is_gfe_map = True)

    for epoch in range(max_epoch):

        for ii in range(1):
            #convert target maps to torch.cuda
            ii_target = rot_gfe[ii].float().cuda()
            ii_input = rot_volume[ii]      
            #perform forward and backward iterations
            optimizer.zero_grad()
            output = model(ii_input)
            loss = criterion(output, ii_target)
            loss.backward()
            optimizer.step()
...
for imap in range(len(map_names_list)):
    ...
    grid = unpad_map(grid, xpad = dim, ypad = dim, zpad = dim)
    ...
    out_name = pdb_ids[0]+"."+ map_names_list[imap]+"_o"
